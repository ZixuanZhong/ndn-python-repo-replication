...
class DeleteHandle():
    def __init__(self, app: NDNApp, command_client : CommandClient, node_name: NonStrictName, repo_prefix : NonStrictName):
        ...
        self.node_delete_prefix = self.node_prefix
        self.node_delete_prefix.append(Component.from_str('delete'))
    ...
    def _on_delete_interest(self, int_name, _int_param, _app_param):
        ...
        logging.info('DELETE Interest Received: {}\n'.format(Name.to_str(int_name)))

        # an invalid Interest which has the exactly same name as the catalog's prefix
        if(len(int_name)==len(self.node_delete_prefix)):
            return

        data_name, hash, desired_copies = self.decode_datainfo(int_name, self.node_delete_prefix)
        self.app.put_data(int_name, content=b'ok', freshness_period=1000)
  
        aio.ensure_future(self.command_client.remove(data_name, hash))


    @staticmethod
    def decode_datainfo(name, node_delete_prefix):
        # get the datainfo name component
        datainfo_name_component = name[len(node_delete_prefix)]
